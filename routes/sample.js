...
function fetchDeployments(namespace, callback) {
    try {
      const kc = new k8s.KubeConfig();
      kc.loadFromDefault();
  ...
  function main() {
    const tenantNamespace = "tenant-74334f-oidev"; // Replace 'your_tenant_namespace' with the actual tenant namespace
    fetchDeployments(tenantNamespace, (error, filteredDeploymentNames) => {
      if (error) {
        console.error('Error:', error);
      } else {
        console.log(filteredDeploymentNames);
        // Do further processing with filteredDeploymentNames if needed
      }
    });
  }
